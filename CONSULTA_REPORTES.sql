...
-- EFECTIVO AL SECO----
SELECT al.tipoServicio,al.idOrden,sum(al.precio*al.cantidad) total from (SELECT l.tipoServicio,o.idOrden,l.precio,l.cantidad,total FROM Orden o inner join OrdenLinea l on o.idOrden=l.idOrden
inner join Pago p on o.idOrden=p.idOrden where p.tipoPago in(1,2) and tipoPago1=0 and tipoPago2=0) al where al.tipoServicio=2 group by al.idOrden;
